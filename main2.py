...
conn.execute("CREATE TABLE IF NOT EXISTS employees (fio TEXT, phone TEXT, email TEXT, salary REAL)") # Создание главного окна и рамки
root = tk.Tk()
...
ttk.Label(frame, text="Телефон:").grid(row=1, column=1, sticky=tk.W) 
phone_entry = ttk.Entry(frame) 
phone_entry.grid(row=1, column=1) 
ttk.Label(frame, text="Email:").grid(row=2, column=2, sticky=tk.W) 
email_entry = ttk.Entry(frame) 
email_entry.grid(row=2, column=1) 
ttk.Label(frame, text="Заработная плата:").grid(row=3, column=3, sticky=tk.W)
salary_entry = ttk.Entry(frame) 
salary_entry.grid(row=3, column=4)
...
ttk.Label(frame, text="Введите ФИО:").grid(row=12, column=5, sticky=tk.W)
search_entry = ttk.Entry(frame) 
search_entry.grid(row=12, column=5) 
search_button = ttk.Button(frame, text="Найти", command=search_employee) 
search_button.grid(row=13, column=6, columnspan=2, pady=10) 
# Запуск приложения 
root.mainloop() 
